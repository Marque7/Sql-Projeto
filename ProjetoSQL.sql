...
/*Para começar vamos criar nossa database(Lembrando que estamos usando a ferramenta SQL WORKBENCH*/
CREATE DATABASE insight_places;

/* Agora vamos criar as tabelas solicitadas pelo cliente */
USE insight_places;
...
/*Agora vamos povoar nossas tabelas com os dados que nos foram disponibilizados após faremos as consultas que nos foram solicitadas com os codigos a seguir*/

/*Primeiro nosso cliente solicita a consulta de avaliacoes dos hoteis com mais de 4 de avaliação*/

SELECT * FROM avaliacoes
WHERE nota >= 4

/*Agora nosso cliente gostaria de conhecer todos os hoteis que estão ativos, lembrando que nosso banco armazena 1 para ativo e 0 para inativo*/

SELECT * FROM hospedagens
WHERE tipo = 'hotel' AND ativo = 1;

/*Nosso cliente agora solicitou uma consulta para saber quanto os clientes gastam em media com hospedagem*/

SELECT cliente_id, AVG(preco_total) AS ticket_medio
FROM reservas
GROUP BY cliente_id;

/*Agora faremos uma busca para ver quanto tempo em media de estadia os hospedes ficam nos hoteis*/
...
/*Agora a empresa solicitou os dados para saber quem são os top 10 locadores que possuem hoteis cadastrados*/
...
/*Pensando em uma limpeza no sistema foi nos solicitado os propietarios que possuem e quatos possuem hoteis e hospedagens desativadas na plataforma*/
...
/*Agora nós faremos algumas alterações solicitadas pelo propietario, estas serão as ultimas atuaçoes no banco*/
